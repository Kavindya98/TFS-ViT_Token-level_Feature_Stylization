...
data_dir=/media/SSD2/Dataset 
output_dir=./Results/${datasets}/Fullset/Fixed_Feature_Clipped_No_Invariant/RandConv
checkpoint_URL =/media/SSD2/Dataset 
for command in delete_incomplete launch
do
    python -u -m domainbed.scripts.sweep ${command} --data_dir=${data_dir} \
    --output_dir=${output_dir}  --command_launcher gpu_0 --algorithms ${algorithms}  \
    --single_domain_gen  --datasets ${datasets}  --n_hparams 1 --n_trials 1  \
    --hparams """{\"batch_size\":64,\"lr\":0.0001,\"continue_checkpoint\":${checkpoint_URL},\"checkpoint_step_start\":0.0,\"resnet_dropout\":0.0,\"consistency_loss_w\":2.0,\"invariant_loss\":true,\"alpha_min\":0.0,\"alpha_max\":1.0,\"weight_decay\":0.0,\"custom_train_val\":true,\"custom_train\":0,\"custom_val\":1,\"resnet18\":false,\"empty_fc\":true,\"fixed_featurizer\":false}"""
done
...
do
    for d_rate in 0.1 0.3 0.5 0.8 # the rate of token selection and replacement (d in the paper) 0.3 0.5 0.8
        ...
        do
            ...
            --hparams """{\"backbone\":\"${backbone}\",\"batch_size\":64,\"lr\":5e-05,\"resnet_dropout\":0.0,\"weight_decay\":0.0,\"fixed_featurizer\":false,\"num_layers\":$n_layers,\"d_rate\":$d_rate,\"alpha\":$alpha}"""
        done
    done
done
...
# nohup bash bash_scripts/TFS-ViT/run_sweep.sh > OUT/PACS/TFSViT/T2T14_PACS_Unsplit_All_diff_val_aug.out 2>&1 &